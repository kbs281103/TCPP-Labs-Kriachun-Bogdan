...
class TramStop {
    // Обов'язкові поля трамвайної зупинки
    private String name;
    private List<Integer> routeNumbers;

    public TramStop(String name, List<Integer> routeNumbers) {
        this.name = name;
    ...
    // Методи `setName()` та `setRouteNumbers()` не використовуються, тому їх можна видалити
...
class Hour extends TramStop {
    // Обов'язкові поля для години
    private final int passengerCount;
    private final String comment;

    public Hour(String name, List<Integer> routeNumbers, int passengerCount, String comment) {
...
class TramStopDatabase {
    ...
    public Hour findHourWithLeastPassengers() {
        Hour minHour = null;
        for (Hour hour : hours) {
    ...
    public Hour findLongestComment() {
        Hour longestCommentHour = null;
        for (Hour hour : hours) {
    ...
    public void displayResults() {
        ...
        Hour minHour = findHourWithLeastPassengers();
        if (minHour != null) {
        ...
        Hour longestCommentHour = findLongestComment();
    ...
    private Hour getHourDataFromUser(Scanner scanner, boolean isEdit) {
        ...
        return new Hour(name, routeNumbers, passengerCount, comment);
    }
}
...
public class lab7_1  {
    ...
    public static void main(String[] args) {
        TramStopDatabase database = new TramStopDatabase();
    ...
    // Завантаження даних із файлу
    private static void loadFromFile(TramStopDatabase database) {
        try (BufferedReader reader = new BufferedReader(new FileReader(FILENAME))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (data.length == 4) {
                    ...
                    Hour hour = new Hour(name, routeNumbers, passengerCount, comment);
    ...
    private static Hour getHourDataFromUser(Scanner scanner, boolean isEdit) {
        System.out.print("Назва зупинки: ");
        String name = scanner.nextLine();
        System.out.print("Номери маршрутів (через кому): ");
        List<Integer> routeNumbers = new ArrayList<>();
    ...
    // Додавання нового запису
    private static void addRecord(TramStopDatabase database, Scanner scanner) {
        Hour newHour = getHourDataFromUser(scanner, false);
        database.addRecord(newHour);
        System.out.println("Новий запис додано.");
    }

    // Редагування існуючого запису
    private static void editRecord(TramStopDatabase database, Scanner scanner) {
        System.out.println("Редагувати запис:");
        System.out.print("Введіть індекс запису для редагування: ");
        int index = Integer.parseInt(scanner.nextLine());
        Hour newHour = getHourDataFromUser(scanner, true);
        database.editRecord(index, newHour);
        System.out.println("Запис редаговано.");
    }

    // Видалення запису за індексом
    private static void deleteRecord(TramStopDatabase database, Scanner scanner) {
